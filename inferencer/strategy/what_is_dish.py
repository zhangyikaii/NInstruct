...
def what_is_dish(data: Dict[str, Any], what_is_dish_num_imgs: int = 3, **kwargs) -> List[Any]:
    results = []
    ...
    for idx, cur_img in enumerate(cur_data_img):
        img_file = os.path.join(IMG_SAVE_PATH, f"{data['id']}_{str(ID_COUNTER)}_{what_is_dish.__name__}_{idx}.jpg")
